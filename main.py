...
from jazzmusicians import JazzProvider

MAX_NUMBER_MUSICIANS_IN_BAND = 10
MAX_INSTRUMENTS_IN_BAND = 5
...
def produce_msgs(security_protocol='SSL',
    ...
    if subject == 'stock':
        ...
        fake.add_provider(JazzProvider)
    while i < nr_messages:
        if subject in ['stock', 'userbehaviour', 'realstock', 'metric', 'bet', 'rolling', 'pizza']:
            message, key = fake.produce_msg()
        else:
            message, key = fake.produce_msg(fake, i, MAX_NUMBER_MUSICIANS_IN_BAND, MAX_INSTRUMENTS_IN_BAND)
...
def main():
    ...
    parser.add_argument('--security-protocol', help='Security protocol for Kafka (PLAINTEXT, SSL, SASL_SSL)',
                        required=True)
    parser.add_argument('--sasl-mechanism',
                        help='SASL mechanism for Kafka (PLAIN, GSSAPI, OAUTHBEARER, SCRAM-SHA-256, SCRAM-SHA-512)',
                        required=False)
    parser.add_argument('--cert-folder',
    ...
    parser.add_argument('--subject',
                        help='What type of content to produce (possible choices are [pizza, userbehaviour, stock, realstock, metric] pizza is the default',
                        required=False)
